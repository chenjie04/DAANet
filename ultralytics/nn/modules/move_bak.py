...
#         # 简化隐藏层维度
#         hidden_dim = max(4, num_experts // 2)  # 确保最小维度
        
#         self.spatial_mixer = nn.Sequential(
#             nn.Linear(num_experts, hidden_dim, bias=False),
#             nn.SiLU(inplace=True),  # 使用inplace节省内存
#             nn.Linear(hidden_dim, num_experts, bias=True),
#             nn.Sigmoid(),
#         )
...
#         pooled = self.avg_pool(x)
#         # 使用flatten更直观
#         weights = self.spatial_mixer(pooled.flatten(2))  # (B, C, num_experts)
#         return weights
...
#         self.expert_norm = nn.InstanceNorm2d(in_channels * num_experts)
#         self.expert_act = nn.SiLU(inplace=True)  # inplace节省内存
...
#         # 合并操作减少中间变量
#         expert_outputs = self.expert_act(self.expert_norm(self.expert_conv(x)))
#         expert_outputs = expert_outputs.view(B, C, self.num_experts, H, W)  # (B, C, A, H, W)
        
#         weights = self.gate(x).view(B, C, self.num_experts, 1, 1)
#         moe_out = (expert_outputs * weights).sum(dim=2)
        
#         return self.out_project(moe_out)
...
#     ):
#         super().__init__()
#         mid_ch = int(in_channels * 0.5)
#         self.in_channels = in_channels
#         self.mid_ch = mid_ch
#         self.inv_sqrt_mid = 1 / math.sqrt(mid_ch)

#         # 共享参数的双路径结构
#         self.qkv = nn.ModuleDict({
#             'W': Conv(in_channels, 1 + 2 * in_channels, k=1, act=True),
#             'H': Conv(in_channels, 1 + 2 * in_channels, k=1, act=True)
#         })
        
#         # 减少最终卷积的输入通道数
#         num_blocks = 2
#         final_conv_in_channels = (2 + num_blocks) * mid_ch
        
#         self.main_conv = Conv(in_channels, mid_ch, k=1, act=True)
#         self.short_conv = Conv(in_channels, mid_ch, k=1, act=True)              
#         self.blocks = nn.ModuleList()
#         for _ in range(num_blocks):
#             self.blocks.append(
#                 MoVE_GhostModule(mid_ch, mid_ch, kernel_size, num_experts)
#             )
        
#         self.final_conv = Conv(final_conv_in_channels, out_channels, k=1, act=True)

#     def _apply_axis_attention(self, x, axis):
#         """通用轴注意力计算"""
#         qkv = self.qkv[axis](x)
#         query, key, value = torch.split(qkv, [1, self.in_channels, self.in_channels], dim=1)
...
#         return value * context_vector * self.inv_sqrt_mid

#     def forward(self, x: torch.Tensor) -> torch.Tensor:
#         residual = x
#         # 全局
#         # 横向处理
#         x_W = self._apply_axis_attention(x, 'W') + residual
#         # 纵向处理
#         x_H = self._apply_axis_attention(x_W, 'H') + x_W

#         residual = x_H
#         # 局部
#         x_short = self.short_conv(x_H)
#         x_main = self.main_conv(x_H)
#         block_outs = []
#         x_block = x_main
#         for block in self.blocks:
#             x_block = block(x_block)
#             block_outs.append(x_block)
#         x_final = torch.cat((*block_outs[::-1], x_main, x_short), dim=1)
#         out = self.final_conv(x_final) + residual
#         return  out

# if  __name__ == "__main__":
#     x = torch.randn(1, 64, 128, 128)
#     model = DualAxisAggAttn_v2(64, 64)
#     y = model(x)
#     print(y.shape)
...
class DualAxisAggAttn(nn.Module):
    ...
    def __init__(
        ...
        self.qkv_W = Conv(
            c1=middle_channels, c2=1 + (2 * middle_channels), k=1, act=True
        )
        self.conv_W = nn.Sequential(
            Conv(c1=middle_channels, c2=middle_channels, k=1, act=True),
            Conv(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        query, key, value = torch.split(
            qkv_W, [1, self.middle_channels, self.middle_channels], dim=1
        )
        # 沿着维度W应用softmax
        ...
        x_W = (
            value * context_vector.expand_as(value) / math.sqrt(self.middle_channels)
            + x_main
        )
        ...
        query, key, value = torch.split(
            qkv_H, [1, self.middle_channels, self.middle_channels], dim=1
        )
        # 沿着维度H应用softmax
...
class TransMoVE(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.in_channels = in_channels
        self.inv_sqrt_mid = 1 / math.sqrt(in_channels)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_W = self._apply_axis_attention(x, 'W') + residual
        residual = x_W
        x_H = self._apply_axis_attention(x_W, 'H') + residual

        # 局部特征提取子层
        residual = x_H
        x_final = self.local_extractor(x_H) + residual

        x_final = self.out_project(x_final)
...
# from ultralytics.nn.modules.tmp_move import TransMoVE
